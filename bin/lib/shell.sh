...
# Puts the 'macstrap' command itself on PATH.
export PATH=\"$MACSTRAP_HOME/bin:\$PATH\"
...
# Puts the 'macstrap' command itself on PATH. fish_add_path is idempotent,
# so re-sourcing this file never stacks duplicate entries.
fish_add_path --global --move --path \"$MACSTRAP_HOME/bin\"
...
# along with Macstrap's own bin so the 'macstrap' command resolves.
\$env.PATH = (\$env.PATH | prepend [\"$MACSTRAP_HOME/bin\" \"$prefix/bin\" \"$prefix/sbin\"] | uniq)
